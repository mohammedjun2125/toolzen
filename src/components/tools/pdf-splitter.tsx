...
import { Upload, X, Loader2, Download, Split, CheckSquare } from 'lucide-react';
...
export default function PdfSplitter() {
    ...
    const handleApplyChanges = async () => {
        ...
        try {
            ...
            const indicesToExtract = Array.from(pagesToExtract).sort((a, b) => a - b);

            const newDoc = await PDFDocument.create();
            const copiedPages = await newDoc.copyPages(pdfDoc, indicesToExtract);
            copiedPages.forEach(page => newDoc.addPage(page));

            const pdfBytes = await newDoc.save();
            saveAs(new Blob([pdfBytes], { type: 'application/pdf' }), `split-${file.name}`);
            toast({ title: 'Success!', description: 'Your new PDF has been downloaded.' });
        } catch (error) {
            console.error(error);
            toast({ variant: 'destructive', title: 'Failed to Split PDF', description: 'An error occurred while modifying the PDF.' });
        } finally {
            setIsProcessing(false);
        }
    };
                ...
                {!file ? (
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            <h3 className="font-semibold text-lg">Select pages to extract from: {file.name}</h3>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {pagePreviews.map((previewUrl, index) => (
                                    <div 
                                        ...
                                        style={{ borderColor: pagesToExtract.has(index) ? 'hsl(var(--primary))' : 'hsl(var(--border))' }}
                                    >
                                        ...
                                        <p className="text-sm text-muted-foreground">Page {index + 1}</p>
                                         {pagesToExtract.has(index) && (
                                            <div className="absolute inset-0 bg-primary/80 flex items-center justify-center">
                                                <Split className="h-16 w-16 text-primary-foreground" />
                                            </div>
                                         )}
                                          <div className="absolute top-2 right-2 bg-background/80 rounded-sm p-1">
                                             <CheckSquare className={`h-5 w-5 ${pagesToExtract.has(index) ? 'text-primary' : 'text-muted-foreground'}`}/>
                                          </div>
                                    </div>
                                ))}
                            </div>
